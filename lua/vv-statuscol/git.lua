-- 行级 git diff：按 buffer 跑 `git diff -U0 HEAD -- <path>`，产出 [lnum] -> 'A'|'C'|'D'
-- 事件驱动刷新（BufReadPost / BufWritePost / FocusGained），不做 per-keystroke 增量
...
function M.refresh(bufnr)
  ...
  vim.system(
    ...
    { text = true },
    vim.schedule_wrap(function(rev)
      if rev.code ~= 0 then
        pending[bufnr] = nil
        markers[bufnr] = nil
        return
